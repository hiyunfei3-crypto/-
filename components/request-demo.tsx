 ...
 useEffect(()=>{
  ...
  const tick=async()=>{
   ...
   try{
    ...
    const next=nextAutoPlayRequest(list,autoReview);
    if(!next)return;
   ...
   }catch(err){setAutoPlay(false);setAutoReview(false);setMessage((err as Error).message)}finally{autoBusy.current=false}
  };
  const timer=setInterval(()=>void tick(),5000);
  void tick();
  return()=>clearInterval(timer);
 },[autoPlay,autoReview,token,pairState,pairCode,list,busyId,refresh]);
