...
import { Check, Copy, MonitorPlay, Heart, Mic2, Play } from 'lucide-react';
...
export function BlindBox({ pool = songs, favorites, toggleFavorite, ready, onRequest, onCopy, copiedId }: { pool?: Song[]; favorites: number[]; toggleFavorite: (song: Song) => void; ready: boolean; onRequest: (song: Song, kind: RequestKind) => void; onCopy:(song:Song)=>void; copiedId:number|null }) {
 ...
 return <section className="blind-box" aria-label="盲盒点歌">
  ...
  <div className="blind-result-slot">{song ? <div className={"blind-result" + (busy ? " blind-card-drawing" : "")}><span>抽中第 {song.id} 首</span><strong>{song.title}</strong>{song.artist && <span className="song-artist">{song.artist}</span>}<div className="inline-song-actions"><button title="点唱" aria-label={`点唱 ${song.title}`} className="song-action sing" disabled={busy} onClick={() => onRequest(song,'sing')}><Mic2/></button><button title="点放" aria-label={`点放 ${song.title}`} className="song-action play" disabled={busy} onClick={() => onRequest(song,'play')}><Play/></button><button title={copiedId===song.id?'已复制':'复制'} aria-label={`复制 ${song.title}`} className="song-action copy" disabled={busy} onClick={()=>onCopy(song)}>{copiedId===song.id?<Check/>:<Copy/>}</button>{song.url&&/^https?:\/\//i.test(song.url)?<a title="打开视频播放" aria-label={`打开 ${song.title} 的视频播放页面`} className="song-action video" href={song.url} target="_blank" rel="noopener noreferrer"><MonitorPlay/></a>:<button title="暂无视频" aria-label={`${song.title} 暂无视频`} className="song-action video" disabled><MonitorPlay/></button>}<button title={favorites.includes(song.id)?'取消喜欢':'喜欢'} className="song-action favorite" disabled={!ready || busy} aria-pressed={favorites.includes(song.id)} onClick={() => toggleFavorite(song)}><Heart fill={favorites.includes(song.id) ? 'currentColor' : 'none'} /></button></div></div> : <p className="blind-placeholder">准备好了吗？摇一摇，看看会遇见哪首歌。</p>}</div>
