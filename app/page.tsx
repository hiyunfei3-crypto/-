...
export default function Home() {
 ...
 const notify = (text: string) => { setCopyMessage(''); setNotice({ text }); };
 ...
 return <Tabs value={view} onValueChange={value => { setView(String(value)); resetFilters(); }} className={'site-shell'+(view==='ranking'||view==='wishes'?' community-view':'')}>
  ...
  <div className={"search-dock"+(view==='ranking'||view==='wishes'?' search-dock-hidden':'')}> <div className="search-box"><Search size={20} /><Input value={query} onChange={e => { setQuery(e.target.value); setPage(1); }} placeholder="搜索歌名或原编号，如 543" aria-label="搜索歌名或原编号" />{query && <button onClick={() => { setQuery(''); setPage(1); }} aria-label="清除搜索"><X size={17} /></button>}</div></div>
